...
def build_pod_spec(
    ...
    main_container = kclient.V1Container(
        ...
        args=[
            ...
            "curl -s -X POST \"http://orchestrator." + AGENT_NAMESPACE + ".svc.cluster.local:8080/api/agents/$AGENT_ID_VAL/complete\" -H 'Content-Type: application/json' -d \"{\\\"agent_id\\\": \\\"$AGENT_ID_VAL\\\", \\\"ticket_id\\\": \\\"$TICKET_ID\\\", \\\"queue_id\\\": \\\"$QUEUE_ID\\\", \\\"lines_added\\\": $LINES_ADDED, \\\"lines_removed\\\": $LINES_REMOVED, \\\"files_changed\\\": $FILES_CHANGED}\" || echo '⚠️ Failed to notify orchestrator'\n"
